...
def edit_note(request):
    ...
    notes = Note.objects.values().filter(user = request.user).order_by('-id')
    user_notes = list(notes)
    return JsonResponse({"status":"1" ,"status_message":"Your Note Upadated Successfully","notes":user_notes})
...
def signup(request):
    ...
    if request.method == 'POST':
        ...
        if (password != re_password):
            ...
            #user = UserModel.objects.create(username= username, password=password,email = email, first_name = name)
            #user.save()
